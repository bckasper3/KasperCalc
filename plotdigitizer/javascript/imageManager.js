...
wpd.imageManager = (function() {
    ...
    function _setImage(image, resumedProject) {
        const loadInstructions = document.getElementById('kc-load-instructions');
        if (loadInstructions) {
            loadInstructions.style.display = 'none';
        }
        const zoomPlaceholder = document.getElementById('kc-zoom-placeholder');
        if (zoomPlaceholder) {
            zoomPlaceholder.style.display = 'none';
        }
        if (_newLoad) {
            wpd.appData.reset();
            wpd.sidebar.clear();
