...
wpd.zoomView = (function() {
    ...
    function drawCrosshair() {
        if (zCrossHair == null) {
            // No image has been loaded yet (init() hasn't run), so there's
            // no zoom crosshair canvas to draw on yet. The chosen color is
            // still saved in crosshairColorText and will apply once one has.
            return;
        }
        zCrossHair.width = zCrossHair.width;
        // crosshairColorText is any valid CSS color (a quick-pick swatch's hex
        // value or a custom one from the color picker), so canvas can use it
        // directly as the stroke style.
        zchCtx.strokeStyle = crosshairColorText || 'black';
