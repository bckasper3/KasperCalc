...
// KasperCalc: every axes type's pixelToLiveString (the mouse-position
// readout under the zoom magnifier) used to call value.toExponential(4)
// unconditionally, so even an ordinary value like 5.2 showed as
// "5.2000e+0". Scientific notation is only actually useful once a value
// is too large or small for a handful of fixed decimals to show it
// meaningfully - everywhere else it's just harder to read at a glance.
wpd.kcFormatLiveNumber = function(value) {
    if (!isFinite(value)) {
        return String(value);
    }
...
wpd.BarAxes = (function() {
    var AxesObj = function() {
        ...
        this.pixelToLiveString = function(pxi, pyi) {
            ...
            return wpd.kcFormatLiveNumber(dataVal[0]);
        };
...
wpd.CircularChartRecorderAxes = class {
    ...
    pixelToLiveString(pxi, pyi) {
        ...
        return timeStr + ', ' + wpd.kcFormatLiveNumber(dataVal[1]);
    }
...
wpd.MapAxes = (function() {
    var AxesObj = function() {
        ...
        this.pixelToLiveString = function(pxi, pyi) {
            var dataVal = this.pixelToData(pxi, pyi);
            return wpd.kcFormatLiveNumber(dataVal[0]) + ', ' + wpd.kcFormatLiveNumber(dataVal[1]);
        };
...
wpd.TernaryAxes = (function() {
    var AxesObj = function() {
        ...
        this.pixelToLiveString = function(pxi, pyi) {
            var dataVal = this.pixelToData(pxi, pyi);
            return wpd.kcFormatLiveNumber(dataVal[0]) + ', ' + wpd.kcFormatLiveNumber(dataVal[1]) + ', ' +
                wpd.kcFormatLiveNumber(dataVal[2]);
        };
    ...
    var AxesObj = function() {
        ...
        this.pixelToLiveString = function(pxi, pyi) {
            ...
            if (isXDate) {
                ...
                rtnString += wpd.kcFormatLiveNumber(dataVal[0]);
            }
            rtnString += ', ';
            ...
            if (isYDate) {
                ...
                rtnString += wpd.kcFormatLiveNumber(dataVal[1]);
            }
            return rtnString;
        };
...
wpd.dataTable = (function() {
    ...
    function setupControls() {
        ...
        if (showDatasets) {
            ...
            // KasperCalc: quick-select buttons (one per dataset) instead of
            // the dropdown, so long as there are few enough to fit
            // comfortably - falls back to the dropdown once there are too
            // many. See kcUpdateDatasetButtons.
            kcUpdateDatasetButtons(datasetNames, selectedDataset.name);
            ...
            // KasperCalc: measurement views always use the dropdown - the
            // quick-select buttons are only for picking between datasets.
            kcUpdateDatasetButtons(null);
        }
    ...
    // KasperCalc: quick-select dataset buttons shown in place of the
    // dropdown when there are few enough datasets to fit comfortably (see
    // kcMaxDatasetButtons). Rebuilt on every setupControls() call so the
    // "selected" highlight stays in sync with whichever dataset is active.
    var kcMaxDatasetButtons = 15;
    var kcDatasetButtonsBound = false;

    function kcEscapeAttr(value) {
        return String(value)
            .replace(/&/g, '&amp;')
            .replace(/"/g, '&quot;')
            .replace(/</g, '&lt;')
    ...
    function kcUpdateDatasetButtons(datasetNames, selectedName) {
        var $dropdownWrap = document.getElementById('data-table-dataset-dropdown-wrap');
        var $buttonsWrap = document.getElementById('data-table-dataset-buttons');
        if ($dropdownWrap == null || $buttonsWrap == null) {
            return;
        }

        if (datasetNames == null || datasetNames.length === 0 ||
            datasetNames.length >= kcMaxDatasetButtons) {
        ...
        datasetNames.forEach(function(name) {
            ...
            html += '<button type="button" class="kc-dataset-select-btn' +
                (isSelected ? ' kc-dataset-select-btn-selected' : '') +
                '" data-dataset-name="' + kcEscapeAttr(name) + '">' +
                kcEscapeAttr(name) + '</button>';
        });
        $buttonsWrap.innerHTML = html;
        $buttonsWrap.style.display = '';

        if (!kcDatasetButtonsBound) {
            kcDatasetButtonsBound = true;
            $buttonsWrap.addEventListener('click', function(ev) {
                var name = ev.target && ev.target.getAttribute('data-dataset-name');
                if (name != null) {
                    kcSelectDataset(name);
                }
            });
        }
    }

    // Same effect as changeDataset() (driven by the dropdown's
    // selectedIndex), but driven by a dataset name instead, for the
    // quick-select buttons above.
    function kcSelectDataset(name) {
        if (selectedDataset == null) {
            return;
        }
        ...
        selectedDataset = ds;
        dataProvider.setDataSource(selectedDataset);
        document.getElementById('data-table-dataset-list').value = name;
        refresh();
    }
    ...
    // KasperCalc: shared by generateCSV/exportJSON/exportExcel below so the
    // "current dataset" name resolution (dataset name, or angles/distances
    // for a measurement table) only lives in one place.
    ...
    function generateCSV() {
        wpd.download.csv(tableText, getSelectedName() + ".csv");
    }

    // KasperCalc: exports just the currently selected dataset/measurement
    // table (same scope as generateCSV above and the on-screen table -
    // whatever's picked in the "Dataset" dropdown and however it's currently
    // sorted/formatted) as a JSON array of row objects keyed by field name,
    // rather than the whole-project wpd.saveResume JSON (axes calibration +
    // every dataset + measurements) already offered elsewhere.
    function exportJSON() {
        if (sortedData == null) {
            return;
        }
        var rows = [];
        for (var rowi = 0; rowi < sortedData.length; rowi++) {
            var rowObj = {};
            for (var coli = 0; coli < dataCache.fields.length; coli++) {
                var val = sortedData[rowi][coli];
                if (dataCache.fieldDateFormat[coli] != null) {
                    val = wpd.dateConverter.formatDateNumber(val, dataCache.fieldDateFormat[coli]);
                }
                rowObj[dataCache.fields[coli]] = val;
            }
            rows.push(rowObj);
        }
        wpd.download.file(JSON.stringify(rows, null, 2), getSelectedName() + ".json");
    }

    // KasperCalc: exports the currently selected dataset/measurement table as
    // an Excel-openable file. There's no XLSX (zip/OOXML) encoder vendored
    // here and adding one just for this would be a lot of weight for one
    // export button, so this uses the well-known trick of handing Excel an
    // HTML <table> with an .xls extension - Excel's own importer opens it
    // directly as a worksheet (recent Excel versions show a one-time "the
    // file format doesn't match the extension" warning before opening it,
    // which is expected for this approach and safe to click through).
    function kcEscapeHTML(value) {
        return String(value)
            .replace(/&/g, '&amp;')
            .replace(/</g, '&lt;')
            .replace(/>/g, '&gt;');
    ...
    function exportExcel() {
        ...
        var coli;
        for (coli = 0; coli < dataCache.fields.length; coli++) {
            html += '<th>' + kcEscapeHTML(dataCache.fields[coli]) + '</th>';
        }
        html += '</tr></thead><tbody>';
        for (var rowi = 0; rowi < sortedData.length; rowi++) {
            html += '<tr>';
            for (coli = 0; coli < dataCache.fields.length; coli++) {
                ...
                html += '<td>' + (val == null ? '' : kcEscapeHTML(val)) + '</td>';
            }
            html += '</tr>';
        }
        html += '</tbody></table>';
        wpd.download.file(html, getSelectedName() + ".xls");
    }
...
wpd.graphicsWidget = (function() {
    ...
    function hoverOverCanvas(ev) {
        ...
        if (extendedCrosshair) {
            $topCanvas.width = $topCanvas.width;
            // KasperCalc: color is normally driven by the "Zoom Dot" swatch
            // picker (kcSetZoomPointPreviewColor sets window.kcExtendedCrosshairColor)
            // rather than being fixed here; falls back to the original color.
            topCtx.strokeStyle = window.kcExtendedCrosshairColor || "rgba(0,0,0, 0.5)";
            topCtx.beginPath();
    ...
    function setZoomImage(ix, iy) {
        ...
        // KasperCalc: xcorr/ycorr above already computed the fractional
        // subpixel offset needed so the zoomed image doesn't visibly snap to
        // whole-image-pixel increments as the mouse moves - but wrapping
        // zxmin+xcorr / zymin+ycorr in parseInt() here threw that fraction
        // away again before it ever reached canvas drawImage (which natively
        // supports fractional coordinates), which is what made panning feel
        // like it was moving in discrete jumps instead of smoothly. Only
        // dropping the parseInt() on the position args, not the width/height
        // ones, since canvas dimensions still make sense as whole pixels.
        wpd.zoomView.setZoomImage(idata, zxmin + xcorr, zymin + ycorr,
    ...
    _renderFolder(data, basePath, isInnerFolder) {
        ...
        for (let i = 0; i < data.length; i++) {
            ...
            if (typeof(item) === "string") {
                ...
                // KasperCalc: icon+label wrapped together so .tree-item can
                // become a flex row (for the right-aligned copy button
                // below) without splitting the icon away from its label -
                // flex would otherwise treat the icon and label as two
                // separate items to space apart.
                htmlStr += "<span class=\"kc-tree-item-content\">";
                ...
                htmlStr += item + "</span>";
                // Small copy-to-clipboard button, right-aligned, on each
                // dataset leaf only (not axes/measurements) - lets the user
                // grab one dataset's data without opening the Acquired Data
                // popup first. event.stopPropagation() keeps the click from
                // also reaching this.$mainElem's own click handler
                // (_onclick), which would otherwise re-select this tree item
                // on every copy click.
                if (basePath === "/" + wpd.gettext("datasets")) {
                    htmlStr += "<button type=\"button\" class=\"kc-copy-dataset-btn has-tooltip\" data-tooltip=\"Copy to clipboard\" data-kc-copy-dataset=\"" +
                        item.replace(/&/g, "&amp;").replace(/"/g, "&quot;").replace(/</g, "&lt;").replace(/>/g, "&gt;") +
                        "\" onclick=\"event.stopPropagation(); wpd.dataExport.copyOneDatasetToClipboard(this.dataset.kcCopyDataset);\"><i class=\"bi bi-clipboard\"></i></button>";
                }
                htmlStr += "</span>";
                ...
                htmlStr += "<span class=\"kc-tree-folder-label\">" + labelKey + "</span>";
                // KasperCalc: "+" shortcut on the three root folders (Axes/
                // Datasets/Measurements) that jumps straight to that
                // folder's "add new" action. event.stopPropagation() keeps
                // the click from also reaching this.$mainElem's own click
                // handler (_onclick), which would otherwise select this
                // folder too.
                if (basePath === "") {
                    if (labelKey === wpd.gettext("axes")) {
                        htmlStr += "<button type=\"button\" class=\"kc-tree-add-btn has-tooltip\" data-tooltip=\"Add Calibration\" onclick=\"event.stopPropagation(); wpd.calibrateAxesDialog.open();\"><i class=\"bi bi-plus-lg\"></i></button>";
                    } else if (labelKey === wpd.gettext("datasets")) {
                        htmlStr += "<button type=\"button\" class=\"kc-tree-add-btn has-tooltip\" data-tooltip=\"Quick Add Dataset\" onclick=\"event.stopPropagation(); wpd.dataSeriesManagement.showAddDataset(true);\"><i class=\"bi bi-plus-lg\"></i></button>";
                    } else if (labelKey === wpd.gettext("measurements")) {
                        htmlStr += "<button type=\"button\" class=\"kc-tree-add-btn has-tooltip\" data-tooltip=\"Add Distance Measurement\" onclick=\"event.stopPropagation(); wpd.tree.addMeasurement(wpd.measurementModes.distance);\"><i class=\"bi bi-plus-lg\"></i></button>";
                    }
                }
                htmlStr += "</span>";
...
wpd.zoomView = (function() {
    ...
    function setZoomImage(imgData, x0, y0, zwidth, zheight, matrix) {
        ...
        // KasperCalc: tempCanvas holds the image at native (unzoomed)
        // resolution and drawImage below scales it up by zoomRatio - without
        // this, the browser's default bilinear smoothing blurs that upscale,
        // which is what made the magnifier (and anything drawn into it,
        // including placed points) look fuzzy instead of crisp. Has to be
        // set here rather than once in init(): `zCanvas.width = zCanvas.width`
        // above resets the whole context state, including this flag, on
        // every call.
...
wpd.XYAxesCalibrator = class extends wpd.AxesCalibrator {
    ...
    align() {
        ...
        // KasperCalc: processCalibration() never checks whether the pixel
        // matrix it inverts is singular, so four collinear/overlapping
        // calibration points silently produce an Infinity/NaN transform
        // (divide-by-zero) instead of calibrate() returning false above.
        // That broken axes object is what later makes "Add Origin Point"
        // appear to do nothing. Catch it here by probing the transform.
        const kcCalibrationTestPixel = axes.dataToPixel(parseFloat(xmin), parseFloat(ymin));
        if (!isFinite(kcCalibrationTestPixel.x) || !isFinite(kcCalibrationTestPixel.y)) {
            wpd.messagePopup.show('Calibration Points Too Close Together',
                'The four calibration points you picked are collinear or overlapping, which makes the axes math divide by zero and breaks the calibration (this also breaks "Add Origin Point"). Please redo the calibration, making sure the X-axis points and Y-axis points are not on the same line.',
                wpd.alignAxes.getCornerValues);
            return false;
        }
...
wpd.alignAxes = (function() {
    ...
    function postProcessAxesAdd(axes, suppressDatasetCreation) {
        ...
        if (axesColl.length === 1 && datasetColl.length === 0 && suppressDatasetCreation != true) {
            ...
            // KasperCalc: was 'Default Dataset', named inconsistently with
            // every dataset added afterward (which count up as "Dataset 1",
            // "Dataset 2", ... via wpd.dataSeriesManagement.showAddDataset).
            // This is always the very first dataset (datasetColl.length ===
            // 0 above), so it's always "Dataset 1" - the getDatasetWithNameCount
            // dedup below still guards the rare case that name is somehow
            // already taken.
            dataset.name = wpd.gettext("dataset") + " 1";
...
// KasperCalc self-hosts WebPlotDigitizer without its cloud backend — there
// is no /api/user or /login route here, so this upstream check always
// failed and redirected every visitor to a 404 page. Disabled.
if (false && !wpd.isOffline() && !wpd.isLocalhost()) {
...
wpd.cloudNewImage = function() {
    return new Promise((resolve, reject) => {
        // KasperCalc self-hosts WebPlotDigitizer without its cloud vision
        // quota-tracking backend — there is no /api/vision/image route
        // here, so this always 405'd on every image load (wpd.isOffline()
        // only recognizes the Electron desktop build as "offline", not a
        // plain self-hosted web page). Same fix as wpd.log()'s disabled
        // /api/analytics ping above.
        resolve();
        return;
        if (wpd.isOffline()) {
            resolve();
...
wpd.dataExport = (function() {
    ...
    // KasperCalc: the table-building loop generateCSV used to do inline is
    // now shared with copyToClipboard/exportExcel below (all three need the
    // exact same combined all-datasets table, just serialized differently),
    // so the axes/columns bookkeeping only lives in one place.
    function buildAllDatasetsTable() {
        let plotData = wpd.appData.getPlotData();
    ...
    function tableToCSVText(table) {
        let csvText = table.header.join(',') + '\n' + table.varheader.join(',') + '\n';
        for (let i = 0; i < table.maxDatapts; i++) {
            csvText += table.valData[i].join(',') + '\n';
        }
        return csvText;
    }
    ...
    function generateCSV() {
        ...
        let table = buildAllDatasetsTable();
        if (table == null) {
            return;
        }
        wpd.download.csv(tableToCSVText(table), "wpd_datasets.csv");
    }

    // KasperCalc: same "each dataset is its own name/header/rows block,
    // stacked one after another" shape as copyToClipboardStacked and
    // exportExcelStacked, but written out as a downloaded .csv file instead
    // of clipboard text or an Excel-openable table. Uses getPerDatasetTables
    // (defined further below) rather than buildAllDatasetsTable above, same
    // reasoning as those two: each dataset's own row count, no padding into
    // a shared side-by-side column layout.
    function generateCSVStacked() {
        wpd.popup.close('export-all-data-popup');
        let tables = getPerDatasetTables();
        if (tables == null) {
            return;
        }
        let csvText = '';
        for (let t = 0; t < tables.length; t++) {
            csvText += tables[t].name + '\n';
            csvText += tables[t].axLab.join(',') + '\n';
            for (let r = 0; r < tables[t].rows.length; r++) {
                csvText += tables[t].rows[r].join(',') + '\n';
            }
            csvText += '\n';
        }
        wpd.download.csv(csvText, "wpd_datasets_stacked.csv");
    }

    // KasperCalc: shared by both copyToClipboard variants below - builds a
    // temporary offscreen textarea to make document.execCommand('copy') work
    // (there's no visible textarea in this dialog to select() the way
    // wpd.dataTable's copyToClipboard does with #digitizedDataTable), then
    // removes it.
    function kcCopyTextToClipboard(text) {
    ...
    // KasperCalc: copies the same combined all-datasets table generateCSV
    // downloads, straight to the clipboard as tab-separated text (rather
    // than comma-separated, so pasting into Excel/Sheets lands one value per
    // cell instead of the whole row in a single cell) - each dataset gets
    // its own group of columns side by side, same shape as the CSV/Excel
    // exports below.
    function copyToClipboard() {
        wpd.popup.close('export-all-data-popup');
        let table = buildAllDatasetsTable();
        ...
        let text = table.header.join('\t') + '\n' + table.varheader.join('\t') + '\n';
        for (let i = 0; i < table.maxDatapts; i++) {
            text += table.valData[i].join('\t') + '\n';
        }
        kcCopyTextToClipboard(text);
    }

    // KasperCalc: per-dataset raw rows (dataset name, axis labels, and just
    // that dataset's own points - no padding to a shared column layout),
    // used by the "stacked" clipboard/Excel exports and the multi-sheet
    // Excel export below, all of which want each dataset as an independent
    // block rather than merged into buildAllDatasetsTable's side-by-side
    // column layout.
    function getPerDatasetTables() {
        let plotData = wpd.appData.getPlotData();
        let dsColl = plotData.getDatasets();
    ...
    // KasperCalc: copies just one dataset's own points to the clipboard
    // (name, axis labels, then rows - no other datasets involved), for the
    // small per-dataset copy button next to each dataset in the left tree.
    // Same tab-separated shape as the other clipboard exports above, just
    // scoped to a single dataset instead of all of them.
    function copyOneDatasetToClipboard(name) {
        let plotData = wpd.appData.getPlotData();
        let dsColl = plotData.getDatasets();
        let ds = null;
        for (let i = 0; i < dsColl.length; i++) {
            if (dsColl[i].name === name) {
                ds = dsColl[i];
                break;
            }
        }
        ...
        let text = ds.name + '\n' + axLab.join('\t') + '\n';
        for (let j = 0; j < pts; j++) {
            let px = ds.getPixel(j);
            text += getValueAtPixel(j, axes, px).join('\t') + '\n';
        }
        kcCopyTextToClipboard(text);
    }

    // KasperCalc: same data as copyToClipboard above, but each dataset is
    // its own block of rows (name, then axis labels, then that dataset's
    // points, then a blank line) stacked one after another instead of
    // side-by-side columns - useful when pasting into a tool that expects
    // one flat list of rows rather than a sparse wide table.
    function copyToClipboardStacked() {
        wpd.popup.close('export-all-data-popup');
        let tables = getPerDatasetTables();
        if (tables == null) {
            return;
        ...
        for (let t = 0; t < tables.length; t++) {
            text += tables[t].name + '\n';
            text += tables[t].axLab.join('\t') + '\n';
            for (let r = 0; r < tables[t].rows.length; r++) {
                text += tables[t].rows[r].join('\t') + '\n';
            }
            text += '\n';
        }
        kcCopyTextToClipboard(text);
    ...
    // KasperCalc: exports the same combined all-datasets table as an
    // Excel-openable file - see the matching exportExcel in wpd.dataTable
    // above for why this is an HTML <table> with an .xls extension rather
    // than a real XLSX binary (no zip/OOXML encoder is vendored here). Each
    // dataset is its own group of columns, same shape as the CSV/clipboard
    // export above.
    function exportExcel() {
        wpd.popup.close('export-all-data-popup');
        let table = buildAllDatasetsTable();
        ...
        for (let i = 0; i < table.header.length; i++) {
            html += '<th>' + kcEscapeHTML(table.header[i]) + '</th>';
        }
        html += '</tr><tr>';
        for (let i = 0; i < table.varheader.length; i++) {
            html += '<th>' + kcEscapeHTML(table.varheader[i]) + '</th>';
        }
        html += '</tr></thead><tbody>';
        for (let i = 0; i < table.maxDatapts; i++) {
            html += '<tr>';
            for (let j = 0; j < table.valData[i].length; j++) {
                let val = table.valData[i][j];
                html += '<td>' + (val === '' || val == null ? '' : kcEscapeHTML(val)) + '</td>';
            }
            html += '</tr>';
        }
        html += '</tbody></table>';
        wpd.download.file(html, "wpd_datasets.xls");
    }

    // KasperCalc: same shape as copyToClipboardStacked - each dataset is its
    // own name row, header row, and data rows, one block stacked after
    // another - but as a single-sheet Excel-openable HTML table instead of
    // clipboard text. colspan on the name row just visually spans that
    // dataset's own column count so it reads as a section heading.
    function exportExcelStacked() {
        ...
        for (let t = 0; t < tables.length; t++) {
            html += '<tr><th colspan="' + tables[t].axLab.length + '">' +
                kcEscapeHTML(tables[t].name) + '</th></tr><tr>';
            for (let c = 0; c < tables[t].axLab.length; c++) {
                html += '<th>' + kcEscapeHTML(tables[t].axLab[c]) + '</th>';
            }
            html += '</tr>';
            for (let r = 0; r < tables[t].rows.length; r++) {
                html += '<tr>';
                for (let c = 0; c < tables[t].rows[r].length; c++) {
                    let val = tables[t].rows[r][c];
                    html += '<td>' + (val == null ? '' : kcEscapeHTML(val)) + '</td>';
                }
                html += '</tr>';
            }
            html += '<tr><td colspan="' + tables[t].axLab.length + '">&nbsp;</td></tr>';
        }
        html += '</tbody></table>';
        wpd.download.file(html, "wpd_datasets_stacked.xls");
    }

    function kcXmlEscape(value) {
        return String(value)
            .replace(/&/g, '&amp;')
            .replace(/</g, '&lt;')
            .replace(/>/g, '&gt;')
    ...
    // KasperCalc: Excel sheet names must be <=31 chars and can't contain
    // : \ / ? * [ ], and every sheet in a workbook needs a unique name -
    // dataset names (freely user-editable elsewhere in the app) satisfy
    // none of that by default, so this sanitizes and de-dupes against every
    // name already used in this export.
    function kcSanitizeSheetName(name, usedNames) {
        let clean = String(name).replace(/[:\\\/\?\*\[\]]/g, '_').slice(0, 31) || 'Sheet';
        let candidate = clean;
        let n = 2;
    ...
    function kcSpreadsheetCell(value) {
        if (typeof value === 'number' && isFinite(value)) {
            return '<Cell><Data ss:Type="Number">' + value + '</Data></Cell>';
        }
        return '<Cell><Data ss:Type="String">' + kcXmlEscape(value == null ? '' : value) +
            '</Data></Cell>';
    }

    // KasperCalc: real multi-sheet export, one dataset per worksheet. The
    // HTML-table-as-.xls trick used by exportExcel/exportExcelStacked above
    // can only ever represent a single sheet (there's no such thing as a
    // "multi-sheet HTML table"), so this instead writes the Excel 2003
    // "SpreadsheetML" XML format, which does support multiple <Worksheet>
    // elements in one plain-text XML file - no zip/OOXML (.xlsx) encoder
    // needed, unlike a real .xlsx file which is a zip archive. Saved with an
    // .xml extension (rather than .xls like the other two) since that's
    // what Excel's own SpreadsheetML association expects, avoiding the
    // "format doesn't match extension" warning the .xls-flavored exports
    // show.
    function exportExcelMultiSheet() {
        ...
        for (let t = 0; t < tables.length; t++) {
            let sheetName = kcSanitizeSheetName(tables[t].name, usedNames);
            xml += ' <Worksheet ss:Name="' + kcXmlEscape(sheetName) + '">\n  <Table>\n   <Row>';
            for (let c = 0; c < tables[t].axLab.length; c++) {
                xml += kcSpreadsheetCell(tables[t].axLab[c]);
            }
            xml += '</Row>\n';
            for (let r = 0; r < tables[t].rows.length; r++) {
                xml += '   <Row>';
                for (let c = 0; c < tables[t].rows[r].length; c++) {
                    xml += kcSpreadsheetCell(tables[t].rows[r][c]);
                }
                xml += '</Row>\n';
            }
            xml += '  </Table>\n </Worksheet>\n';
        ...
        xml += '</Workbook>\n';
        wpd.download.file(xml, "wpd_datasets.xml");
    }
    ...
    return {
        show: show,
        generateCSV: generateCSV,
        generateCSVStacked: generateCSVStacked,
        copyToClipboard: copyToClipboard,
        copyToClipboardStacked: copyToClipboardStacked,
        copyOneDatasetToClipboard: copyOneDatasetToClipboard,
        exportExcel: exportExcel,
        exportExcelStacked: exportExcelStacked,
        exportExcelMultiSheet: exportExcelMultiSheet,
        exportToPlotly: exportToPlotly
    };
})();
/*
...
wpd.download = (function() {
    ...
    return {
        json: json,
        csv: csv,
        file: textFile // KasperCalc: generic text-file download for formats (e.g. .xls) that don't get their own named helper here
    };
})();
/*
    WebPlotDigitizer - web based chart data extraction software (and more)
...
wpd.log = function() {
    ...
    // KasperCalc self-hosts WebPlotDigitizer without its analytics backend —
    // there is no /api/analytics route here, so this always 404'd on every
    // page load. Disabled.
    return;
